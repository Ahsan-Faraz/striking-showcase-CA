...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: `${process.env.NEXT_PUBLIC_APP_URL ?? ""}/api/og/image?id=${params.username}`,
          width: 1200,
          height: 630,
        },
      ],
...
export default async function PublicPortfolioPage({ params }: Props) {
  ...
  const highlightVideo = athlete.media.find(
    (m) => m.isFeatured && m.type === "video",
  );
  const photos = athlete.media.filter(
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {(
              [
                ...
                [
                  "Ball Speed",
                  athlete.ballSpeed ? `${athlete.ballSpeed} mph` : null,
                ],
                ["PAP", athlete.pap],
                ["Axis Tilt", athlete.axisTilt ? `${athlete.axisTilt}°` : null],
                [
                  "Axis Rotation",
                  athlete.axisRotation ? `${athlete.axisRotation}°` : null,
                ],
              ] as const
            ).map(([label, val]) =>
              val != null ? (
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
              {athlete.arsenal.map((b) => (
                ...
                >
                  ...
                  {b.brand && (
                    <p className="text-sm text-gray-400">{b.brand}</p>
                  )}
