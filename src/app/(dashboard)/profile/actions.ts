...
async function getAthleteProfileId(): Promise<{ userId: string; profileId: string } | { error: string }> {
  const user = await verifySession();
  if (!user) return { error: 'Not authenticated' };
  if (!user.athleteProfile) return { error: 'No athlete profile found' };
  return { userId: user.id, profileId: user.athleteProfile.id };
}

export async function savePersonalInfo(formData: unknown): Promise<ActionResult> {
...
export async function saveAcademicInfo(formData: unknown): Promise<ActionResult> {
  const auth = await getAthleteProfileId();
  if ('error' in auth) return { error: auth.error };
