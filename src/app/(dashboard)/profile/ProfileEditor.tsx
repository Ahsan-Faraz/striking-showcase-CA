...
const LABEL =
  "block text-xs font-medium text-[var(--text-tertiary)] mb-1.5 uppercase tracking-wider font-mono";
      ...
      switch (activeTab) {
        case "personal":
          result = await savePersonalInfo({
            ...
            usbcClub: profile.usbcClub || null,
          });
          break;
        case "bowling":
        ...
        case "privacy":
          result = await savePrivacy({
            profileVisibility: profile.profileVisibility as
              | "PUBLIC"
              | "PRIVATE",
  ...
  const handlePhotoUpload = async (file: File) => {
    ...
    try {
      ...
      if (data.url) {
        ...
        // Persist profile photo URL to the athlete profile record
        await fetch("/api/athletes/me", {
          method: "PUT",
          ...
          <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        update("classYear", parseInt(e.target.value))
                      }
                    >
                      {[
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  className="input"
                  value={profile.coachContact ?? ""}
                  onChange={(e) => update("coachContact", e.target.value)}
                />
              </div>
              <div>
                ...
                <input
                  className="input"
                  value={profile.proShop ?? ""}
                  onChange={(e) => update("proShop", e.target.value)}
                />
              </div>
              <div>
                ...
                <input
                  className="input"
                  value={profile.bowlingCenter ?? ""}
                  onChange={(e) => update("bowlingCenter", e.target.value)}
                />
              </div>
            </div>
          </CardContent>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {(["RIGHT", "LEFT"] as const).map((hand) => (
                    <button
                      key={hand}
                      onClick={() => update("dominantHand", hand)}
                      className={cn(
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  className="input"
                  value={profile.pap ?? ""}
                  onChange={(e) => update("pap", e.target.value)}
                  placeholder="e.g. 5 1/2 over, 1/2 up"
                />
              </div>
              <div>
                ...
                <input
                  className="input"
                  value={profile.usbcId ?? ""}
                  onChange={(e) => update("usbcId", e.target.value)}
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  className="input"
                  value={profile.ncaaStatus ?? "Pending"}
                  onChange={(e) => update("ncaaStatus", e.target.value)}
                >
        ...
        <Card className="animate-in">
          <CardContent className="space-y-6">
            ...
            <div className="border-t border-[var(--border-primary)] pt-6">
              ...
              <div className="flex flex-wrap gap-2">
                {["Northeast", "Southeast", "Midwest", "Southwest", "West"].map(
                  (region) => (
                    <button
                      key={region}
