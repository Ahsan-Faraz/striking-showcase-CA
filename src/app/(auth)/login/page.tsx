...
function LoginContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Sync Prisma record and check if onboarding is needed
      const syncRes = await fetch("/api/auth/sync", { method: "POST" });
      if (syncRes.ok) {
        const syncData = await syncRes.json();
        if (syncData.data?.needsOnboarding) {
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <button
          ...
          style={{
            background:
              "linear-gradient(135deg, #C9A84C 0%, #E0C878 50%, #C9A84C 100%)",
          }}
        >
          {loading ? (
      ...
      <p className="text-sm text-[var(--text-tertiary)] text-center mt-6">
